...
Quaternion::Quaternion(float theta, Vector4D axis)
{
	Vector4D axisn = axis.Normalize_Quaternions();
...
Vector4D Quaternion::normalizeVec4(Vector4D& v)
{
	Vector4D vn;
...
void Quaternion::toAngleAxis(float& angle, Vector4D& axis)
{
...
Matrix4D Quaternion::GLRotationMatrix()
{
